...
public class NioEchoServer implements Runnable {
	...
	private static final Logger logger = Logger.getLogger(NioEchoServer.class
			.getName());
	...
	public NioEchoServer(InetSocketAddress inetSocketAddress) {
		if (inetSocketAddress == null) {
			throw new NullPointerException(
					"Parameter inetSocketAddress must not be null");
		} else if (inetSocketAddress.isUnresolved()) {
			throw new IllegalArgumentException("Unresolved inetSocketAddress: "
					+ inetSocketAddress);
		} else {
	...
	public void run() {
		try (ServerSocketChannel serverSocketChannel = ServerSocketChannel.open();) {
			...
			throw new RuntimeException(
					"Encountered an I/O error. Bailing out.", e);
		}
	}
	...
	private void onAccept(SelectionKey selectionKey) throws IOException {
		...
		SelectionKey clientSelectionKey = socketChannel.register(
				selectionKey.selector(), SelectionKey.OP_READ);
	...
	public static void main(String[] args) throws IOException {
		if (args.length == 0) {
			...
			final InetSocketAddress inetSocketAddress = new InetSocketAddress(
					hostname, port);
			final NioEchoServer nioEchoServer = new NioEchoServer(
					inetSocketAddress);
